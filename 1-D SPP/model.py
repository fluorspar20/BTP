...
class SSE(nn.Module):
    ...
    def gen_a_update(self,x_a):
        ...
        # Encode
        a, p = self.getSTFT(x_a)
        h_a,mag_a,p_a = self.gen_a.encode(a,p)
        # Decode
        x_a_recon,mag_a_recon = self.gen_a.decode(h_a,p_a)
        # Loss
    ...
    # Train the MAE
    def gen_b_update(self,x_b,x_b_noise,fs_noise,x_b_clean,notnoise):
        self.opt_b.zero_grad()
        
        # Encode
        a,p = self.getSTFT(x_b)
        
        h_b,mag_b,p_b = self.gen_b.encode(a,p)
        # Decode (within domain)
        x_b_recon, mag_b_recon = self.gen_b.decode(h_b,p_b)
        # Decode (cross domain)
        x_ba, mag_ba = self.gen_a.decode(h_b,p_b)
        # Encode again
        a,p = self.getSTFT(x_ba)
        h_b_recon,mag_b_recon,p_b_recon = self.gen_a.encode(a,p)
        # Decode again
        x_bab, mag_bab = self.gen_b.decode(h_b_recon,p_b_recon)
        ...
        #cur_x_b = x_b[0].cpu().numpy()
        #cur_fs_noise = fs_noise[0].cpu().numpy()
        #mag_b_noise = estimate_noise(cur_x_b, cur_fs_noise)
        #mag_b_noise = mag_b_noise.reshape(1,513,126)
        
        mag_b_noise_final = np.zeros((self.config['b_size'], 513, 126))
        
        for i in range(self.config['b_size']):  # iterate through every sample in the batch
            cur_x_b = x_b[i].cpu().numpy()
            cur_fs_noise = fs_noise[i].cpu().numpy()
            mag_b_noise_estimate_cur = estimate_noise(cur_x_b, cur_fs_noise)
          	# mag_b_noise = np.vstack((mag_b_noise, mag_b_noise_cur.reshape(1,513,126)))
            mag_b_noise_estimate_cur = mag_b_noise_estimate_cur.T
          
            cur_x_b_clean = x_b_clean[i].cpu().numpy()
            frame_len = 1024
            hop_size = 256
            noverlap = frame_len - hop_size
            
            f_clean, t_clean, Zxx_clean = sg.stft(cur_x_b_clean, fs=self.config['sr'], nfft=1024, nperseg=frame_len, noverlap=noverlap)
            f_noisy, t_noisy, Zxx_noisy = sg.stft(cur_x_b, fs=self.config['sr'], nfft=1024, nperseg=frame_len, noverlap=noverlap)
            
            Zxx_clean = np.abs(Zxx_clean)
            Zxx_clean = Zxx_clean.T
            Zxx_noisy = np.abs(Zxx_noisy)
            Zxx_noisy = Zxx_noisy.T
            
            E_f = np.zeros(126)
            for j in range(126):
                E_f[j] = np.sum(np.abs(Zxx_clean[j])**2)
            E_f = E_f / np.linalg.norm(E_f)
            SPP_1D = E_f > self.config['spp_threshold']
            final_noise_estimate_cur = np.zeros((126,513))
            
            for j in range(126):
                if(SPP_1D[j]):
                    final_noise_estimate_cur[j] = mag_b_noise_estimate_cur[j]
                else:
                    final_noise_estimate_cur[j] = Zxx_noisy[j]
                    
            final_noise_estimate_cur = final_noise_estimate_cur.T
            mag_b_noise_final[i] = final_noise_estimate_cur
	  
	  	    #mag_b_noise = np.vstack((mag_b_noise, final_noise_estimate_cur.reshape(1,513,126)))
        
        mag_b_noise_final = torch.from_numpy(mag_b_noise_final).cuda(x_b.data.get_device())
        mag_b_noise_final = mag_b_noise_final.type(torch.cuda.FloatTensor)

	  	    
        a,p = self.getSTFT(x_b)
        h_b_noise, mag_b_noise_b, p_b_noise_b = self.gen_b.encode(mag_b_noise_final, p)
        x_b_noise_pow_recon, mag_b_noise_recon = self.gen_b.decode(h_b_noise, p_b_noise_b)
        
        x_ba_noise_clean_recon, mag_ba_noise_clean_recon = self.gen_a.decode(h_b_noise, p_b_noise_b)
        
        self.loss_noise = self.recon_criterion(mag_b_noise_recon, mag_b_noise_final) + self.config['noise_clean_ba_w']*self.recon_criterion(mag_ba_noise_clean_recon,0)
  ...
  # ---------- modified code ---------
    def forward(self,x,option,x_noise=None,fs_noise=None,x_clean=None,notnoise=None):
        if option == 'a':
            return self.gen_a_update(x)
        elif option == 'b':
            return self.gen_b_update(x,x_noise,fs_noise,x_clean,notnoise)
        elif option == 'eval': 
