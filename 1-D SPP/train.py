...
parser.add_argument('--pretrain_clean', type=bool, default=True)
parser.add_argument('--pretrain_CAE', type=str, default='cae.pkl')
parser.add_argument('--pretrain', type=bool, default=True)
parser.add_argument('--pretrain_MAE', type=str, default='model_ep750_th0.01.pkl')
...
train_noise_files, test_noise_files = get_all_noise_files(config['noise_dataset']), get_all_noise_files(config['noise_dataset'])
...
for epoch in range(1,config['epochs_b']+1):
    ...
    # --------- modified code -----------
    for i, (audio_b,audio_b_clean,audio_b_noise,fs_noise,notnoise) in enumerate(train_B_dataloader):
        # --------- original code -----------
        # loss,mag_b,mag_b_recon,mag_ba,x_b_recon,x_ba = trainer(audio_b.float().to(device),'b',notnoise.float().to(device))
        # --------- modified code -----------
        # print(fs_noise)
        loss,mag_b,mag_b_recon,mag_ba,x_b_recon,x_ba = trainer(audio_b.float().to(device),'b',audio_b_noise.float().to(device),fs_noise.float().to(device),audio_b_clean.float().to(device), notnoise.float().to(device))
        wandb.log({'train_loss': loss})
        if i == len(train_B_dataloader)-1 and epoch%5 == 0:
            end = time.time()
            ...
            if epoch%50 == 0:
                with open('{}/model_ep{}_th{}.pkl'.format(BASE_DIR,epoch+750,config['spp_threshold']), 'wb') as output:
                    pickle.dump(trainer, output, pickle.HIGHEST_PROTOCOL)
...
# Save model
with open('{}/model_th{}.pkl'.format(BASE_DIR, config['spp_threshold']), 'wb') as output:
    pickle.dump(trainer, output, pickle.HIGHEST_PROTOCOL)
