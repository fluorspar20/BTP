...
class SSE(nn.Module):
    ...
    # Train the MAE
    def gen_b_update(self,x_b,x_b_noise,fs_noise,notnoise):
        ...
        mag_b_noise = np.zeros((self.config['b_size'], 513, 126))
        ...
        for i in range(self.config['b_size']):  # iterate through every sample in the batch
          cur_x_b_noise = x_b_noise[i].cpu().numpy()
          _, _, Zxx_noise = sg.stft(cur_x_b_noise, fs=self.config['sr'], nfft=1024, nperseg=frame_len, noverlap=noverlap)
          mag_b_noise[i] = np.abs(Zxx_noise)
        
        mag_b_noise = torch.from_numpy(mag_b_noise).cuda(x_b.data.get_device())
        mag_b_noise = mag_b_noise.type(torch.cuda.FloatTensor)

        a,p = self.getSTFT(x_b)
        h_b_noise, mag_b_noise_b, p_b_noise_b = self.gen_b.encode(mag_b_noise, p)
        ...
        self.loss_noise = self.recon_criterion(mag_b_noise_recon, mag_b_noise) + self.config['noise_clean_ba_w']*self.recon_criterion(mag_ba_noise_clean_recon,0)
  ...
  # ---------- modified code ---------
    def forward(self,x,option,x_noise=None,fs_noise=None,notnoise=None):
        if option == 'a':
            return self.gen_a_update(x)
        elif option == 'b':
            return self.gen_b_update(x,x_noise,fs_noise,notnoise)
        elif option == 'eval': 
