...
parser.add_argument('--pretrain_MAE', type=str, default='oracle_noise_ep400.pkl')
...
for epoch in range(1,config['epochs_b']+1):
    ...
    for i, (audio_b,audio_b_clean,audio_b_noise,fs_noise,notnoise) in enumerate(train_B_dataloader):
        ...
        # --------- modified code -----------
        # print(fs_noise)
        loss,mag_b,mag_b_recon,mag_ba,x_b_recon,x_ba = trainer(audio_b.float().to(device),'b',audio_b_noise.float().to(device),fs_noise.float().to(device), notnoise.float().to(device))
        wandb.log({'train_loss': loss})
        if i == len(train_B_dataloader)-1 and epoch%5 == 0:
            ...
            if epoch%100 == 0:
                with open('{}/oracle_noise_ep{}.pkl'.format(BASE_DIR,epoch+400), 'wb') as output:
                    pickle.dump(trainer, output, pickle.HIGHEST_PROTOCOL)
...
# Save model
with open('{}/model_oracle_noise.pkl'.format(BASE_DIR), 'wb') as output:
    pickle.dump(trainer, output, pickle.HIGHEST_PROTOCOL)
